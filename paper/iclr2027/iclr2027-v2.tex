...
Parts of the theory are machine-checked in Lean against Mathlib, in seven files with no unproven step and no axiom beyond the standard ones: the tolerance condition of Theorem~\ref{thm:dist}(c), the semiorder of Theorem~\ref{thm:pref}(b), the hull law of Theorem~\ref{thm:repr}(b), the whole-space case of the uniqueness in Theorem~\ref{thm:repr}(c), the reversal example, and the two statements on shared-representation regret in Appendix~ef{app:gates}, with Ky Fan's principle taken as a named hypothesis.
