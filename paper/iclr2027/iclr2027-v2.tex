...
\pending{G-Eval probability-weighted scoring, authority and cue biases in LLM judges, reasoning and judge bias: prior-art check and credit before this section is final.}
...
\pending{Written after the sealed grades: one stimulus family with an exact quality scale; judges that failed anti-vacuity; the bars fixed from one judge's pilot; the flip at one gap and one decoration.}
Nothing here is a claim about human evaluators.

\subsection*{AI use statement}
\pending{Adapt from the G3 version after the text settles.}
...
\pending{From G3c PREREG Section 6 and verify/*.json: sampling against the first-token distribution, the digit tree against real sampling and against brute force, the chunked tree for the 14B judge.}
