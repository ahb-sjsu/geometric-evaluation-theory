...
\date{September 2026. \emph{Draft 0.2.} Every statement is labeled proved, defined, or posited. Nothing labeled posited is a claim.}
...
The theory separates what the world supplies, the consequence map, from what the evaluator owns, the metric, the ideal, the budget, and the admissible set, and that separation makes representation a theorem rather than a tautology.
For a fixed world, a preference is representable exactly when a semidefinite feasibility problem has a solution, every representable preference obeys a hull law that no action is strictly worse than every action whose consequences surround it, and on an open set of consequences the metric is identified up to a positive scale and the ideal up to the metric's null directions.
...
The nearest neighbor with a budget is rational inattention and the limited-attention models of choice~\cite{sims2003,caplin2015,masatlioglu2012,matejka2015}, in which a cost of information about the state, or a limit on the alternatives attended to, produces consideration sets, stochastic choice, and violations of revealed preference.
The budget of the present theory is a different quantity.
It limits the resolution of consequences under the evaluator's metric, not the acquisition of information about states, and its predictions, a semiorder threshold equal to the budget, a reversal between two fixed actions when the resolved directions change, and the hull law of Section~\ref{sec:repr}, are not statements those models make.
The two are not rivals.
The belief $\mu$ on the states is where an information cost would enter the evaluation object, and the budget $B$ is where resolution enters, and one object can carry both.
...
Section~\ref{sec:derive} derives distinctions, preference, and choice from it and proves the representation and uniqueness theorems that the separation makes possible.
...
With $C$ fixed by the world, the question of which preferences an evaluator can hold becomes a representation theorem, and the question of what its choices reveal becomes a uniqueness theorem, both in Section~\ref{sec:repr}.
...
\subsection{Representation and uniqueness}
\label{sec:repr}

The split of Definition~\ref{def:split} turns the existence of a representing utility from an assumption into a theorem with content.
Throughout this subsection $X$ is a point, $\Y=\R^{m}$, the consequences $y_{a}=C(a)$ are fixed by the world, and the evaluator chooses a positive semidefinite $G$ and an ideal $t$.
For a pair $(G,t)$ write $q(y)=(y-t)^{\top}G(y-t)$, so that $c(a)=q(y_{a})$.

\begin{definition}[Representable preference]
\label{def:repr}
A weak order $\succsim$ on $A$ is representable in the world $(A,C)$ if there is a positive semidefinite $G$ and an ideal $t$ with $a\succsim b$ if and only if $q(y_{a})\le q(y_{b})$.
A semiorder is representable at budget $\varepsilon$ if there are $G$, $t$ with $a\succ b$ if and only if $\sqrt{q(y_{b})}>\sqrt{q(y_{a})}+\varepsilon$.
\end{definition}

\begin{theorem}[Representation for a fixed world]
\label{thm:repr}
Let $A$ be finite with consequences $y_{a}\in\R^{m}$, and let $\succsim$ be a weak order on $A$ with strict part $\succ$ and indifference $\sim$.
\begin{enumerate}
\item[(a)] \emph{Characterization.} $\succsim$ is representable if and only if there exist a positive semidefinite $G$, a vector $b$ in the range of $G$, and a scalar $\gamma$ such that the quadratic $Q(y)=y^{\top}Gy+b^{\top}y+\gamma$ satisfies $Q(y_{a})<Q(y_{b})$ whenever $a\succ b$ and $Q(y_{a})=Q(y_{b})$ whenever $a\sim b$. The set of such $(G,b,\gamma)$ is the feasible set of a semidefinite program with $|A|(|A|-1)/2$ linear constraints, so representability is decidable and, when it holds, an open cone of representations exists in the strict case.
\item[(b)] \emph{Hull law.} If $\succsim$ is representable, then for every action $a$ and every finite set $S\subseteq A$ with $y_{a}$ in the convex hull of $\{y_{s}:s\in S\}$, there is $s\in S$ with $a\succsim s$. In words, no action is strictly worse than every action whose consequences surround it. The law holds for every evaluation whose cost is a convex function of the consequence, not only a quadratic one.
\item[(c)] \emph{Affinely independent consequences.} If $\{y_{a}\}$ is affinely independent, so that $|A|\le m+1$, then every weak order on $A$ is representable, and by a positive definite $G$.
\item[(d)] \emph{Consequences on a line.} If all $y_{a}$ lie on one line, then $\succsim$ is representable if and only if it is single-peaked along the line, with the peak at the point nearest the ideal.
\end{enumerate}
\end{theorem}

\begin{proof}
(a) Expanding, $q(y)=y^{\top}Gy-2t^{\top}Gy+t^{\top}Gt$, which is $Q$ with $b=-2Gt$ and $\gamma=t^{\top}Gt$, and $b$ lies in the range of $G$. Conversely, given $Q$ with $b$ in the range of $G$, choose $t$ with $Gt=-b/2$, and then $Q(y)=q(y)+\gamma-t^{\top}Gt$ differs from $q$ by a constant, so the two order $A$ identically. The constraints are linear in the entries of $(G,b,\gamma)$ for fixed consequences, the positive semidefinite cone is convex, and the range condition is the linear constraint that $b$ be orthogonal to the kernel of $G$, which for $G$ in the relative interior of a face of the cone is a linear subspace, so the problem is a semidefinite feasibility problem. In the strict case the constraints are strict inequalities, and a feasible point remains feasible under small perturbation and under positive scaling.
(b) $q$ is convex because $G$ is positive semidefinite. If $y_{a}=\sum_{s\in S}\lambda_{s}y_{s}$ with $\lambda_{s}\ge0$ summing to one, then $q(y_{a})\le\sum_{s}\lambda_{s}q(y_{s})\le\max_{s}q(y_{s})$, so $a\succsim s^{*}$ for a maximizer $s^{*}$. The argument used only convexity.
(c) Take $G=I$. Then $q(y_{a})=\|y_{a}\|^{2}-2t^{\top}y_{a}+\|t\|^{2}$, and the map $t\mapsto\bigl(q(y_{a})-\|t\|^{2}\bigr)_{a\in A}$ is affine with linear parts $-2y_{a}^{\top}t$ and constants $\|y_{a}\|^{2}$. Affine independence of the $y_{a}$ makes the pairwise differences of these affine functions linearly independent as functions of $t$, so every prescribed pattern of strict inequalities and equalities among them is attained by some $t$, since a finite family of linearly independent affine functionals takes every value pattern.
(d) With the line parametrized by arc length $s$ and unit direction $v$, $q(y(s))=g(s-s_{0})^{2}+h$ with $g=v^{\top}Gv\ge0$, $s_{0}$ the parameter of the projection of $t$ onto the line in the $G$-geometry, and $h\ge0$. For $g>0$ this is single-peaked with peak at $s_{0}$, and for $g=0$ it is constant, which is the degenerate single-peaked order. Conversely, a single-peaked weak order with peak between two consecutive actions or at one of them is attained by choosing $s_{0}$ at the peak and $g>0$, with $h$ arbitrary, since the order of $(s_{a}-s_{0})^{2}$ then matches the order of distances from the peak.
\end{proof}

Part (b) is the first law of the theory that a scalar utility does not state.
A utility on $A$ can rank an action below all of its neighbors in consequence space.
An evaluator with a metric cannot, because a distance to an ideal has convex sublevel sets, and the law is checkable on data whenever the consequence map is known.
A closed-form combinatorial characterization of representability, beyond the feasibility problem of part (a), is open.

\begin{theorem}[Uniqueness]
\label{thm:unique}
Let $(G_{1},t_{1})$ and $(G_{2},t_{2})$ be two evaluations on $\R^{m}$ with $G_{1}\ne0$, and suppose they induce the same weak order on a nonempty open connected set $U$ of consequences, that is, $q_{1}(y)\le q_{1}(y')$ if and only if $q_{2}(y)\le q_{2}(y')$ for all $y,y'\in U$.
Then $G_{2}=\lambda G_{1}$ for some $\lambda>0$ and $G_{1}(t_{2}-t_{1})=0$.
In particular, when $G_{1}$ is positive definite, the ideal is identified exactly and the metric up to a positive scale, and when $G_{1}$ is singular the ideal is identified up to the null directions of the metric, which the evaluator does not read.
\end{theorem}

\begin{proof}
Same weak order on $U$ means $q_{2}=\varphi\circ q_{1}$ on $U$ for a strictly increasing $\varphi$ on $q_{1}(U)$, which is an interval because $U$ is connected and $q_{1}$ is continuous.
Take any line segment in $U$ along which $q_{1}$ is not constant. Along it, $q_{1}(s)=\alpha(s-s_{0})^{2}+\beta$ with $\alpha>0$, symmetric about $s_{0}$, and $q_{2}(s)$ is a quadratic in $s$ equal to $\varphi(q_{1}(s))$, hence also symmetric about $s_{0}$, so $q_{2}(s)=\alpha'(s-s_{0})^{2}+\beta'$ and $\varphi(u)=(\alpha'/\alpha)(u-\beta)+\beta'$ is affine on the interval $q_{1}$ traces along the segment, with positive slope because $\varphi$ is increasing.
Two such segments through a common point trace intervals that overlap in a neighborhood of that point's value, and an affine function is determined by its values on an interval, so $\varphi$ is one affine function $\varphi(u)=\lambda u+\mu$ with $\lambda>0$ on all of $q_{1}(U)$, since $U$ is connected and the segments can be chained.
Then $q_{2}=\lambda q_{1}+\mu$ on the open set $U$, and two polynomials that agree on an open set agree identically, so comparing quadratic parts $G_{2}=\lambda G_{1}$ and comparing linear parts $G_{2}t_{2}=\lambda G_{1}t_{1}$, which is $G_{1}(t_{2}-t_{1})=0$.
\end{proof}

\begin{remark}
The uniqueness theorem is the theory's identification result.
It says what the learned-geometry protocol of the observer-representation paper can recover from an evaluator's choices, namely the metric up to scale and the ideal up to the metric's kernel, and no more.
On a finite set of consequences the same conclusion needs the consequences to be rich enough, which is the affine-independence condition of Theorem~\ref{thm:repr}(c) applied to the differences that the choices reveal, and that condition is the design bound of the protocol.
The theorem and its bound have been tested once, under a sealed registration, on synthetic evaluators in two to four dimensions, with the representation program of Theorem~\ref{thm:repr}(a) as the estimator and errors scored against the chance level of guesses from the evaluator prior.
From a weak order on 64 consequences above the dimension the metric and the ideal's range component were recovered to a few percent of chance, with the error falling at every step as the battery grew, while at or below the design bound, off the span of a battery confined to a subspace, and in the kernel of a singular metric nothing was recovered, in every one of twelve cells.
A semiorder that withholds the fifth of pairs closest in distance identifies more slowly, to between 5 and 25 percent of chance at the same battery.
\end{remark}
...
The following are the theory's predictions, each stated so that a measurement can fail it, and three of them have been measured under sealed registrations, as recorded below.
\begin{enumerate}
...
\item[(iv)] \emph{Hull law.} For any evaluator whose consequence map is known, no action is chosen below every action whose consequences surround it, in the sense of Theorem~\ref{thm:repr}(b). A choice pattern in which an action inside the convex hull of a set of consequences is strictly rejected in favor of none of them fails the theory for every convex metric of evaluation, not only for a quadratic one.
\item[(v)] \emph{Menu-dependent admissibility.} Where an obligation is created by the availability of an action, choice must violate the weak axiom in the pattern of Theorem~\ref{thm:admiss}, and the violation must disappear when the obligation-creating action is removed from every menu.
\end{enumerate}
...
The first has been run once, under a sealed registration, on a language model, Qwen2.5-7B-Instruct, used as a scorer that is shown a target and a value and reports how far apart they are, so that the induced order compares reported distances and equal reports are indifference.
Three budgets were set independently, the number of decimals at which values are shown, the number of tokens the report may use, and the precision of the weights.
Along the rendering ladder the threshold was 0.87, 0.087, 0.0086 and 0.00087 at 0, 1, 2 and 3 decimals, against rendering steps of 1, 0.1, 0.01 and 0.001.
Along the report ladder it was 8.8, 0.87, 0.87, 0.087, 0.0088 and 0.00087 at 1 to 6 tokens, against the resolutions of 10, 1, 1, 0.1, 0.01 and 0.001 that those tokens afford with two digits before the decimal point.
The threshold tracked each output budget at 0.87 to 0.88 of the step, which is where the 90 percent level of a scorer that rounds to a grid falls, and pairs separated by 20 were ordered the same way in every cell at an accuracy of at least 0.985.
Reducing the weights to 8 or 4 bits left every threshold where the full weights put it, so on this evaluator the two output budgets are the resolution budget of the semiorder and the weight precision is not a resolution budget at all.
The sealed verdict is a pass.
The hull law requires only a known consequence map and observed choices, and it is the cheapest test of the theory.
It has been run once, under a registration sealed before the ratings were read, on the 1972 American National Election Study, where respondents placed Nixon, McGovern, Wallace and the two parties on seven-point issue scales and rated them on feeling thermometers.
On the registered pair of scales, 489 respondents had a menu in which the law could bind, and 50 percent of them violated it at least once, against 66 percent when each respondent's ratings are shuffled among that respondent's own objects, with no shuffle in 200 reaching the observed rate.
...
The rankings sit well below the geometric base rate of violation and well above zero, so on this world the law holds as a population tendency and fails as the deterministic statement of Theorem~\ref{thm:repr}(b) for about half of the respondents.
...
It has been run once, under a sealed registration, on the grouped-query attention of a language model, with the three query heads of a group as the evaluators of one key cache and shared codes of rank 2, 4 and 8 in 128 dimensions.
At those ranks the measured attention loss is 20 to 330 times the second-order prediction, so the regret formula and its bound were not tested, and the sealed verdict is indeterminate.
The ordering claim survives on the measured losses alone, the leading eigenspace of the summed geometries beat every random code and the key-covariance code in 32 of 32 cases, while a head's own leading eigenspace was the best code for that head in fewer than half the cases, so outside the quadratic regime Theorem~\ref{thm:incompat} orders the shared codes and does not identify the private optimum.
A second sealed run perturbed one key at a time by a small isotropic error in the discarded subspace, which is the object the formula predicts, and there the measured loss matched the second-order prediction at the median, a ratio of 1.02 to 1.05 that did not move with the perturbation size up to a tenth of a whitened unit, while 64 draws per cell left a 20 percent scatter that missed the registered per-triple bar, so that verdict is indeterminate as well.
Inside the quadratic regime the regret formula, the bound and the optimality of the compromise follow from the recovered operators by the theorem, so what a measurement can add there is whether the loss is quadratic with the recovered operator, and it is.
The fifth requires menus with and without an obligation-creating action, which is a choice experiment.
...
Representability is semidefinite feasibility; the hull law is necessary for every convex evaluation; affinely independent consequences represent every order; a line represents exactly the single-peaked orders & proved & Theorem~\ref{thm:repr} \\
The metric is identified up to scale and the ideal up to the metric's kernel from the order on an open set & proved & Theorem~\ref{thm:unique} \\
A closed-form combinatorial characterization of representability & open, not claimed & Section~\ref{sec:repr} \\
...
\bibitem{sims2003}
C.~A.~Sims, ``Implications of rational inattention,'' \emph{J. Monetary Econ.}, vol.~50, no.~3, pp.~665--690, 2003, doi: 10.1016/S0304-3932(03)00029-1.

\bibitem{caplin2015}
A.~Caplin and M.~Dean, ``Revealed preference, rational inattention, and costly information acquisition,'' \emph{Amer. Econ. Rev.}, vol.~105, no.~7, pp.~2183--2203, 2015, doi: 10.1257/aer.20140117.

\bibitem{masatlioglu2012}
Y.~Masatlioglu, D.~Nakajima, and E.~Y.~Ozbay, ``Revealed attention,'' \emph{Amer. Econ. Rev.}, vol.~102, no.~5, pp.~2183--2205, 2012, doi: 10.1257/aer.102.5.2183.

\bibitem{matejka2015}
F.~Mat\v{e}jka and A.~McKay, ``Rational inattention to discrete choices: A new foundation for the multinomial logit model,'' \emph{Amer. Econ. Rev.}, vol.~105, no.~1, pp.~272--298, 2015, doi: 10.1257/aer.20130047.
