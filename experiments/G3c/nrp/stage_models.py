"""Stage one model's weights onto the G3c volume from a 1-CPU, 2 GiB pod. Runs inside the NRP job.
...
def main() -> int:
    ...
    ap.add_argument("--model-id", required=True); ap.add_argument("--revision", default=None)
    ap.add_argument("--dest", required=True)
    a = ap.parse_args()
    dest = Path(a.dest); dest.mkdir(parents=True, exist_ok=True)
    if (dest / "STAGED.json").exists():
        print("already staged", dest); return 0
    api = HfApi()
    info = api.model_info(a.model_id, revision=a.revision)
    files = [f for f in api.list_repo_files(a.model_id, revision=info.sha) if f.endswith(PATTERNS)]
    local = Path("/tmp/dl"); manifest = {"model_id": a.model_id, "revision": info.sha, "files": {}}
    t0 = time.time(); total = 0
    for f in sorted(files):
        p = Path(hf_hub_download(a.model_id, f, revision=info.sha, local_dir=str(local)))
        ...
        total += size
        p.unlink()
        print(f"{f} {size / 1e6:.0f} MB written at {size / 1e6 / max(time.time() - t1, 1e-3):.1f} MB/s", flush=True)
    shutil.rmtree(local, ignore_errors=True)
    manifest["seconds"] = round(time.time() - t0, 1); manifest["bytes"] = total
    ...
    print("STAGED", a.model_id, info.sha, f"{total / 1e9:.2f} GB in {manifest['seconds']} s, read {manifest['read_mb_per_s']} MB/s")
    return 0
