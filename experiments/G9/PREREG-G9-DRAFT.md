...
A unit `(e, c)` enters only when every one of the four actions is observed at least five times,
so that a share is estimated from something rather than from nothing.

What that device costs, measured rather than assumed. The self-test in `g9_hull_law.py`
generates menus whose rankings come from an actual convex quadratic cost, so the true violation
rate is zero by construction, then replaces the true ranking with one revealed by choice share
at a given floor and counts the violations that appear. At the floor of five the device
manufactures violations in 5.2 percent of testable menus and disagrees with the true ranking in
612 of 957 menus. At ten the manufactured rate is 1.3 percent and at twenty-five it is 0.5
percent.

The direction of that bias is the reason the floor stays at five. Manufactured violations raise
the observed rate, and the primary bar passes only when the observed rate sits well below the
null, so the device makes a pass harder to obtain and cannot produce one. A pass is therefore
not explained by it. A miss partly can be, and this paragraph exists so that reading is
available before the number is seen rather than after.

The floor is five and not ten because the floor sweep in the same file measured what a higher
floor costs. At five the primary plane has 402 testable units, at ten it has 184, at fifteen 112
and at twenty 69. Only five clears the anti-vacuity bar of 300 that this gate inherits from G2.
Raising the floor to buy a cleaner ranking would empty the gate instead.
...
Sensitivity, carrying no bar. The same statistic is reported at a minimum count of ten per
action, where the revealed ranking is cleaner and the anti-vacuity bar is not met, so a reader
can see whether the verdict turns on the floor. A verdict that reverses between the two is
reported as such and the gate is graded INDETERMINATE regardless of which side the sealed floor
falls on.
...
**Probe 6, the runner's own self-test and floor sweep, `g9_hull_law.py --selftest` and
`--floorsweep`.** Three checks and one sweep, run on Atlas 2026-09-19.

The checker reproduces the theorem. Rankings generated by an actual convex quadratic cost give
zero violations in 140 testable menus, so a violation this gate reports is not the checker
misreading a convex evaluator. A uniformly random ranking gives 0.185, near the quarter the
geometry predicts, so the statistic can move.

The third check is the one G2 did not need, and it found the cost of the revealed ranking
recorded in Section 4. The first version of this self-test also failed on its own expectation
band rather than on the code, because the band was written from intuition and the geometry says
the base rate must sit near a quarter. The band was corrected to a principled one and the reason
is in the source.

The floor sweep is in Section 4. Its finding is that the anti-vacuity bar of 300 and the cleanest
available ranking cannot both be had, and the registration takes the bar and declares the bias.

**Outstanding before seal.** Nothing. Six probes fill Sections 2, 3, 4 and 7, every floor in
Section 6 is either met or its plane is declared vacuous in advance, and the analysis code passes
its synthetic self-test on Atlas.
