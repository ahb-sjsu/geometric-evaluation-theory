...
def grade(res: dict, tol: float) -> dict:
    cfg = res["config"]
    cells = {(c["weight_precision"], c["decimals"]): c for c in res["cells"]}
    wps = list(cfg["weight_precisions"]); decs = [int(d) for d in cfg["decimals_ladder"]]
    dmax = max(decs)
    floor = cells[(wps[0], dmax)]["threshold"]
    out = {"tol": tol, "floor_full_precision_max_decimals": floor, "thresholds": {}, "P1": {}, "P2": {}, "P3": {}, "gate": None}
    ...
    for wp in wps:
        ...
        for d in decs:
            step = 10.0 ** (-d)
            if math.isfinite(floor) and step >= 3 * floor:
