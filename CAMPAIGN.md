...
| G1 | Machine-checked core | G0 | FIRST PASS BUILT 2026-09-07 (five files, no sorry; general uniqueness and Ky Fan not yet checked) |
...
Record, 2026-09-07. First pass built on Atlas at `/archive/ahb-sjsu/geometric-evaluation-theory/lean` against Mathlib v4.32.2 with the textbook formalization's packages. Checked: the semiorder theorem (both axioms, irreflexivity, transitivity, trichotomy, the zero-threshold weak order), the tolerance witness and the chain condition for transitivity, the hull law for every convex cost, one-dimensional uniqueness of the ideal with scale freedom, and the reversal example with the no-common-utility statement. One Mathlib name (`lt_or_le`) and one deprecated tactic (`push_neg`) had to be replaced; no statement changed. Not yet checked: uniqueness in general dimension, the semidefinite characterization, single-peakedness on a line, and Ky Fan's equality case for the incompatibility theorem. Axiom audit in `lean/Axioms.lean`: the clean pass (commit 2b51dd9, 8661 jobs, zero warnings, no `sorry`) reports every audited theorem depending only on `propext`, `Classical.choice`, and `Quot.sound`.
